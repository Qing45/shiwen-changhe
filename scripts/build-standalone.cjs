...
function PoemsRiverPage() {
  ...
  // 法上无碰撞，但在 2250% 画布下移动端像素间距仅 ~31px（亮斑+文字视觉
...
function markCleared(keyword, corpus, band) {
  ...
  var keywordList = corpus === 'primary' ? PRIMARY_KEYWORDS : KEYWORDS;
...
function CharModeBody(props) {
  ...
  var charKeywords = props.charKeywords;
        ...
        return (
          <div key={tier} style={{ marginBottom: compact ? 24 : 36 }}>
            ...
            <div style={{
              ...
              {words.map(function(kw) {
                var globalIdx = charKeywords.indexOf(kw);
